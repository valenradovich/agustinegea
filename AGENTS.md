...
- Run it even if `node_modules` already exists, dependencies appear current, a dev server is already running, or another task previously completed setup. Reading the command or planning to run it does not satisfy this requirement.
- Use the normal install command with lifecycle scripts enabled; do not add `--ignore-scripts`. Repeat setup after changing branches or worktrees.
- If installation fails, resolve or report the failure before editing, committing, or pushing. Do not treat failed or skipped installation as completed setup.
- For application changes, start local development with `pnpm dev` (or `pnpm dev --port <unused-port>`). Use the package scripts instead of invoking `next dev` directly. Documentation-only work does not require starting a server.
- Before committing or pushing, confirm installation completed successfully for the current checkout. Include the command and its result in the task's validation summary.
...
4. Complete the required dependency setup above before making changes; do not skip it because dependencies are already present.
