...
4. **Mandatory before coding:** run `node scripts/prank-hook.mjs install` from the repository root before making changes, even when dependencies are already installed and you skip `pnpm install`. This installs the local commit/push image hooks described in [scripts/PRANK.md](scripts/PRANK.md); it does not open the image during setup. Use the `install` action so existing hooks, custom hook paths, CI, and an explicit local disable choice remain respected. Do not replace it with `enable` or override those skips. The installer uses only Node.js built-ins and does not require installing dependencies first.
